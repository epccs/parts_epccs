...
# * Creates part **first**, then pushes BOM with **retry + manual input**
...
# * pushes suppliers/manufacturers/price breaks like parts
#
# example usage:
# python3 ./api/json_to_inv-templates.py "Furniture/Tables/*_Table"
# python3 ./api/json_to_inv-templates.py "**/*" --force --clean-dependencies
# --------------------------------------------------------------
...
def create_category_hierarchy(folder_path, parent_pk=None):
    parts = os.path.relpath(folder_path, "data/templates").split(os.sep)
    cur = parent_pk
    for name in parts:
        if name == ".":
            continue
...
# ----------------------------------------------------------------------
# Push one template part
# ----------------------------------------------------------------------
def push_template_part(part_path, force_ipn=False, force=False, clean=False):
...
# ----------------------------------------------------------------------
def push_bom(parent_pk: int, bom_path: Path, level: int = 0):
    indent = " " * level
    try:
...
def main():
    parser = argparse.ArgumentParser(
        description="Push template parts + single-level BOM from data/templates/ to InvenTree"
    )
    parser.add_argument(
        "patterns", nargs="*",
        help="Glob patterns (default: all under data/templates)"
    )
    ...
    for key, flist in key_to_files.items():
        for f, rev in flist:
            push_template_part(f, args.force_ipn, args.force, args.clean_dependencies)
