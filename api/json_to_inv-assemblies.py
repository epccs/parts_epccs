#!/usr/bin/env python3
# file name: json_to_inv-assemblies.py
# version: 2025-11-15-v1
# --------------------------------------------------------------
# Push to inventree assemblies + BOMs from data/assemblies/
#
...
def create_category_hierarchy(folder_path, parent_pk=None):
    parts = os.path.relpath(folder_path, "data/assemblies").split(os.sep)
    cur = parent_pk
    for name in parts:
        if name == ".":
            continue
...
# Push one assembly
# ----------------------------------------------------------------------
def push_assembly(part_path, force_ipn=False, force=False, clean=False):
...
def main():
    parser = argparse.ArgumentParser(
        description="Push assemblies + BOMs from data/assemblies/ to InvenTree"
    )
    parser.add_argument(
        "patterns", nargs="*",
        help="Glob patterns (default: all under data/assemblies)"
    )
    ...
    for key, flist in key_to_files.items():
        for f, rev in flist:
            push_assembly(f, args.force_ipn, args.force, args.clean_dependencies)
